...
`easy-install.py` подготавливает сервер к развертыванию Compass On‑premise: удаляет AppArmor, проверяет ресурсы, устанавливает системные зависимости, выпускает сертификаты и запускает основной инсталлятор `install.py`.
